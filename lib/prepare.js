...
async function sDistPackage(setupPy, distDir) {
  try {
    ...
    throw new Error(`failed to build source archive: ${error}`)
  }
}
...
async function bDistPackage(setupPy, distDir) {
  try {
    ...
    throw new Error(`failed to build wheel: ${error}`)
  }
}
...
async function preparePoetry(pluginConfig, nextRelease, logger) {
  ...
  try {
    ...
    throw new Error(`Failed to run "poetry version ${nextRelease.version}": ${error}`)
  }
}
